...
# VI at Nivel scale using pandas
...
h_res = []
for h in ["Amazonas", "Pacífico", "Titicaca"]:

    df_vi = data_all[data_all.nivel == h] # analysis by Nivel
    p_hist = bdk_cal.P.median()
    pet_hist = bdk_cal.PE.median()
    ae_hist = bdk_cal.AE.median()
    wa_hist = p_hist - ae_hist
    ...
    df_vi.PE.values = np.round(np.linspace(0, 50, 100), 1)
    df_vi.P.values = np.round(np.linspace(-50, 50, 100), 1)
    h_res.append(df_vi.to_dataset())

pickle.dump(h_res, open("./scripts/to_output/VI_nivel_scale.pkl", "wb"))


"""
# VI at basin scale using all
...
for h in range(0,len(bdk_cal)):
    ...
    df_vi["AE_sp"] = df_vi["Ei"] * df_vi["P_sp"]
    df_vi["WA_model"] = df_vi["P_sp"] - df_vi["AE_sp"]
    df_vi["VI"] = np.round((df_vi["WA_model"] - wa_hist) / wa_hist * 100, 3)
    df_vi = np.round(df_vi.groupby(["PE", "P"]).VI.median(), 3)
    df_vi = df_vi.to_xarray()
    df_vi.PE.values = np.round(np.linspace(0, 50, 100), 1)
    df_vi.P.values = np.round(np.linspace(-50, 50, 100), 1)
...
pickle.dump(h_res, open("/home/adrian/Documents/wa_budyko_datasets/pickle/VI_basin_scale_using_all.pkl", "wb"))
